...
def parse_args():
    parser = argparse.ArgumentParser(description='Train GNN models for NR binding prediction.')
    ...
    parser.add_argument('--params-path', required=True, help='JSON file containing best hyperparameters.')
    parser.add_argument('--save-dir', required=True, help='Directory for model checkpoints and result CSVs.')
...
def predict_model(model, data_loader, device, model_name):
    model.eval()
    all_probs = []
    all_labels = []
...
def evaluate_predictions(all_labels, all_probs, task_names):
    ...
    for i in range(num_tasks):
        ...
        all_valid_preds.extend((all_probs[mask, i] >= 0.5).astype(int))
...
def evaluate_model(model, test_loader, device, model_name, task_names):
    all_labels, all_probs = predict_model(model, test_loader, device, model_name)
    return evaluate_predictions(all_labels, all_probs, task_names)

def train_and_evaluate(model_name, params, dataset, task_names, in_channels, edge_dim,
                       protein_encodings=None, device=None, num_epochs=200, patience=5,
                       save_dir='./gnn_results_protein/seed_5673_esm2'):
    print(f"\n{'='*60}")
    ...
    model.load_state_dict(torch.load(os.path.join(save_path, f'{model_name}_best.pth'), map_location=device))

    print(f"\nEvaluating on test set with fixed 0.5 threshold...")
