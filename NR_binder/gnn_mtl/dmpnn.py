...
class DMPNN(MessagePassing):
    def __init__(self, node_in_channels, edge_in_channels, hidden_channels, num_layers):
        ...
        self.norm = nn.LayerNorm(hidden_channels, eps=1e-5, elementwise_affine=True)
    ...
    def message_aggregation(self, H, edge_index, rev_edge_index, num_nodes):
        ...
        M_sum = torch.zeros(num_nodes, H.shape[1], dtype=H.dtype, device=H.device)
        M_sum.scatter_add_(0, index_torch, H)
        M_all = M_sum[edge_index[0]]
    ...
    def final_message_aggregation(self, H, edge_index, num_nodes):
        ...
        M = torch.zeros(num_nodes, H.shape[1], dtype=H.dtype, device=H.device)
        M.scatter_add_(0, index_torch, H)
